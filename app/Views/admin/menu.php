...
<body>
    <div class="flex h-screenn overflow-hidden">
        <?= view("components/sidebar", ["active" => "menu"]) ?>
        <div class="flex-1 h-screenn overscroll-y-auto p-5">
                ...
                <?php foreach ($menus as $key => $item) : ?>
                    ...
                    <div class="rounded-md shadow-md bg-zinc-100 overflow-clip flex-1 group">
                        <div class="w-full h-48 overflow-clip relative">
                            <!-- tools -->
                            <div class="absolute group-hover:opacity-100 opacity-0 transition-all ease-out flex items-center justify-center gap-x-3 w-full h-full bg-zinc-900 bg-opacity-60 z-10">
                                <!-- edit button -->
                                <a href="<?= base_url("admin/menu/{$item["id"]}/edit/") ?>" class="cursor-pointer text-zinc-50 hover:text-zinc-300" title="Edit">
                                ...
                                <button class="text-zinc-50 hover:text-red-400" title="Delete" onclick="return confirm('yakin ingin menghapus menu <?= $item['nm_menu'] ?>')  ">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                            ...
                            <img src="<?= base_url("uploads/" . $item["gambar"]) ?>" alt="" loading="lazy" class="object-cover h-full group-hover:scale-110 transition-transform ease-out mx-auto">
                        </div>
                        <div class="px-3 py-1">
